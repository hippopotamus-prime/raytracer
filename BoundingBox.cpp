...
bool BoundingBox::IntersectRay(const Point& src, const Vector& ray, double near_cull)
{
    // TODO - speed this up using the stuff from the Pluecker paper

    // Basic idea - consider the box as the intersection of three "slabs" in space.
    // The ray intersects each slab twice, at a near plane and a far plane.
    // If the first of the far plane intersections comes before the last near plane
    // intersection, the ray misses the box.
    ...
    if(ray.x != 0.0)
    {
        ...
        largest_near = (to_min_plane < to_max_plane) ? to_min_plane : to_max_plane;
        smallest_far = (to_min_plane < to_max_plane) ? to_max_plane : to_min_plane;

        // See if those are the final near intersection or the first far intersection
        // (well, of course they are at this point, since we did x first)
    }
    else if((src.x < x) || (src.x > x + dx))
    {
        // Oh, the ray doesn't actually intersect the x planes...  then if the ray
        // doesn't start out between them, it can't possibly hit the box.
    ...
    if(ray.y != 0.0)
    {
        ...
        to_near_plane = (to_min_plane < to_max_plane) ? to_min_plane : to_max_plane;
        to_far_plane = (to_min_plane < to_max_plane) ? to_max_plane : to_min_plane;
    ...
    // So, not only does the first far plane intersection have to be farther away than
    // the last near plane intersection, but it also has to be in front of the ray
    // starting point...
